...
class VQGanTransformer(nn.Module):
    def __init__(self):
        ...
        model_filename = 'vqgan.transformer.drin.model.ckpt'
        config_filename = 'vqgan.transformer.drin.config.yml'
        ...
        # transformer_config['block_size'] = 1024
        ...
        # model_filename = 'vqgan.1024.model.ckpt'
        # config_filename = 'vqgan.1024.config.yml'
        # config_vae = OmegaConf.load(str(Path(CACHE_PATH) / config_filename))
        # vae = VQModel(**config_vae.model.params)
        # state_vae = torch.load(str(Path(CACHE_PATH) / model_filename), map_location = 'cpu')['state_dict']
        # vae.load_state_dict(state_vae, strict = True)
        # st()  # verified they are the same
